...
- ✅ **เพิ่ม `alt`/`aria-label`/`aria-hidden` ที่ขาดหายทั่วโปรเจกต์ (accessibility audit)** — เดิม audit เจอว่าไม่มีการใส่ attribute เหล่านี้เลยทั้งโปรเจกต์ ทำให้ screen reader อ่านหน้าเว็บไม่ได้ความ (รูปไม่มีคำอธิบาย, ปุ่มไอคอนอ่านไม่รู้ว่ากดแล้วทำอะไร) ตอนนี้แก้แล้ว 3 ส่วน:
  1. **`<img>` ไม่มี `alt` (28 จุด)** — ทั้งใน `.html` ตรงๆ (รูปพรีวิวไฟล์แนบ, ลายเซ็นผู้ตรวจแต่ละขั้นใน `portfolio-teacher.html`/`portfolio-admin.html`, avatar ครูที่เลือก) และใน `.js` ที่ render HTML ด้วยการต่อ string (avatar navbar/admin/ครูใน `shared/common.js`, `js/admin-role.js`, `js/portfolio-admin.js`, `js/room-admin.js`, รูปแนบแจ้งซ่อมใน `js/repair-user.js`/`js/repair-admin.js`) — จุดที่ `alt` มาจากข้อมูลผู้ใช้ (ชื่อคน) ใช้ `esc2()` ครอบก่อนเสมอตาม convention เดิมของโปรเจกต์ (กัน XSS แบบเดียวกับตอนแทรก HTML text อื่นๆ)
  2. **`<svg>` ไอคอนตกแต่งไม่มี `aria-hidden` (12 จุด)** — โลโก้ Google/LINE ในปุ่มล็อกอิน/การ์ดต่างๆ ใน `index.html` และจุด timeline (`.ct-dot`) ใน `js/profile.js` ทุกจุดมีข้อความข้างๆ สื่อความหมายซ้ำอยู่แล้ว จึง mark เป็น `aria-hidden="true"` แทนที่จะใส่คำอธิบายซ้ำ
  3. **ปุ่มที่มีแค่ไอคอน `data-lucide` ไม่มีข้อความไม่มี `aria-label` (49 จุด)** — กระจายอยู่ทั่วโปรเจกต์ (ปุ่มปิด modal ทุกหน้า, ปุ่มลบ `trash-2`, ปุ่มเปลี่ยนเดือน/ปีใน calendar, ปุ่มลบไฟล์แนบ/แท็กที่เลือก, ปุ่มดูรายละเอียด) ใส่ `aria-label` ตามการกระทำจริงของปุ่มนั้นๆ (เช่น `"ปิด"`, `"ลบ"`, `"เดือนถัดไป"`, `"ปีก่อนหน้า"`) — **ต้องระวังจุดที่มีทั้งปุ่มก่อนหน้า/ถัดไปในการ์ดเดียวกัน** (`changeMonth`/`changeYear`/`stepPeriodYear`/`pubChangeMonth`) เพราะ icon เดียวกัน (`chevron-left`/`chevron-right`) ใช้ซ้ำกับ context อื่นด้วย (เช่น `chevron-right` ใช้ทั้งปุ่ม "เดือนถัดไป" และปุ่ม "ดูรายละเอียด") ต้องเช็คว่า `onclick` เป็นฟังก์ชันอะไรก่อนตั้ง label ทุกครั้ง อย่าใช้ icon name เดามั่ว
  - **Convention ใหม่ที่ต้องทำต่อจากนี้**: ดูหัวข้อ "Conventions ที่ต้องรู้ก่อนแก้โค้ด" → บรรทัด Accessibility ด้านบน — ทุกครั้งที่เพิ่ม `<img>`/`<svg>`/ปุ่มไอคอนใหม่ ต้องใส่ attribute เหล่านี้ตั้งแต่แรก ไม่ต้องรอ audit รอบหน้า

- ✅ **แก้ hardcoded hex color → CSS variable ใน inline style ของทุกไฟล์ `.html`** — เดิม audit เจอ `style="..."` ที่มี hex color hardcode ตรงๆ **786 จุด** (นับเฉพาะใน inline style attribute ของไฟล์ `.html` ทั้ง 15+1 ไฟล์ ไม่รวม `<style>` block และไม่รวม JS ที่ render HTML ด้วย string) ซึ่งขัดกับ convention ที่เขียนไว้ด้านบน ("ห้าม hardcode hex ใหม่ในหน้าเว็บ") ทำให้เวลาผู้ดูแลระบบเปลี่ยนธีมสีผ่าน `settings.html` สีที่ hardcode ไว้จะไม่เปลี่ยนตาม (bug เชิงภาพที่มองไม่เห็นจนกว่าจะลองเปลี่ยนธีม) ตอนนี้แก้แล้ว:
  1. เขียนสคริปต์ไล่แทนที่ hex code ในทุก `style="..."` attribute ด้วย `var(--token-name)` ที่ค่าตรงกันเป๊ะกับตัวแปรใน `shared/styles-new.css` (`:root`) — แทนที่ไปทั้งหมด **741 จุด** จาก 786 จุด
  2. เพิ่ม **token ใหม่** ใน `shared/styles-new.css` (หัวข้อ "Extended Neutral / Accent Palette") สำหรับสีที่ถูก hardcode ซ้ำบ่อยแต่ไม่เคยมี token รองรับมาก่อน เช่น `--text-dark` (#0f172a, เดิม hardcode 46 จุด), `--text-mid` (#475569), `--red-bright` (#ef4444), `--violet` (#7c3aed), `--blue-accent` (#2563eb) ฯลฯ ดูรายชื่อ token ทั้งหมดในไฟล์ CSS โดยตรง
