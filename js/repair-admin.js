...
/* ── ตัวกรองช่วงเวลา: ใช้ระบบกลาง registerPeriodScope/renderPeriodBar/filterByPeriod
     จาก shared/common.js (เดิมเป็นโค้ดในไฟล์นี้ ถูกสกัดออกไปเป็นของกลางให้หน้าอื่นดึงไปใช้ได้)
     ใช้ร่วมกันทั้งหน้า "การแจ้งซ่อม" (repair:active), "ประวัติทั้งหมด" (repair:history)
     และ "พิมพ์รายงาน" (repair:print) — แต่ละจุดเลือกช่วงเวลาเป็นอิสระจากกัน ── */
registerPeriodScope('repair:active',  { onChange: function() { renderStats(); renderActivePanel(); } });
registerPeriodScope('repair:history', { onChange: function() { renderTable(); } });
registerPeriodScope('repair:print',  {});
...
/* getPeriodRange/renderPeriodBar/renderPeriodPopoverBody/positionPeriodPopover/
   rerenderPeriodScope/reopenPeriodPopover/setPeriodMode/setPeriodMonth/setPeriodYear/
   stepPeriodYear — ย้ายไปเป็นของกลางใน shared/common.js แล้ว (ใช้ key แบบ 'repair:active' ฯลฯ) */
...
function renderStats() {
  renderPeriodBar('repActivePeriodBar', 'repair:active');
  var scoped = filterByPeriod(allRepairs, 'repair:active');
...
function renderActivePanel() {
  ...
  var periodScoped = filterByPeriod(allRepairs, 'repair:active');
...
function renderTable() {
  ...
  if (!tbody) return;
  renderPeriodBar('repHistoryPeriodBar', 'repair:history');

  var list = filterByPeriod(allRepairs, 'repair:history').filter(function(r) {
...
/* ══════════════════════ EVENT HANDLERS ══════════════════════ */
/* filterByPeriod/togglePeriodPopover/global click&scroll listeners — ย้ายไปอยู่ใน shared/common.js แล้ว */
...
function openPrintReportModal() {
  resetPeriodScope('repair:print');
  printTechnicianFilter = '';
  printStatusFilter = '';
  var statusSel = document.getElementById('printStatusFilter');
  if (statusSel) statusSel.value = '';
  renderPeriodBar('printPeriodBar', 'repair:print');
  renderPrintTechnicianFilterSelect();
  openModal('printReportModal');
}
/* periodLabelText() ใช้ตัวกลางจาก shared/common.js (คืน label ทั่วไปแบบ "ทั้งหมด (ทุกช่วงเวลา)")
   หน้านี้อยากได้คำว่า "รายสัปดาห์นี้" เฉพาะตอนเลือกโหมดสัปดาห์ จึงห่อเพิ่มอีกชั้นบางๆ ไว้ */
function repairPeriodLabelText(key) {
  return getPeriodState(key).period === 'week' ? 'รายสัปดาห์นี้' : periodLabelText(key);
}
...
function doPrintReport() {
  ...
  var list = filterByPeriod(allRepairs, 'repair:print');
...
/* การ์ดของรายการเดียว ใช้ร่วมกันทั้งโหมด "สถานะเดียว" และโหมด "ทั้งหมด แยกตามสถานะ"
   (ใช้ printReportItemHTML() จาก shared/common.js — รูปแบบการ์ดกลางของทั้งระบบ) */
function renderPrintItem(r, idx) {
  var cat = getCategoryMeta(r.category);
  var meta = getStatusMeta(r);

  var badges = [];
  if (r.priority === 'urgent') badges.push({ label: 'เร่งด่วน', color: 'urgent' });
  badges.push({ label: meta.label, color: meta.color });

  return printReportItemHTML({
    index: idx,
    title: r.title || '(ไม่มีหัวข้อ)',
    badges: badges,
    infoRows: [
      { label: 'ผู้แจ้ง',         value: (r.reporterName || r.reporterEmail || '-') + (r.reporterPosition ? (' (' + r.reporterPosition + ')') : '') },
      { label: 'เบอร์โทร',        value: r.reporterPhone || '-' },
      { label: 'วันเวลาที่แจ้ง',  value: fmtDate(r.createdAt) },
      { label: 'สถานที่',         value: r.location || '-' },
      { label: 'หมวดหมู่',        value: cat.label },
      { label: 'ผู้รับผิดชอบ',    value: r.technician || '-' }
    ],
    descBoxes: [
...
function openPrintWindow(list) {
  var periodLabel = repairPeriodLabelText('repair:print');
  var statusLabel = PRINT_STATUS_LABELS[printStatusFilter] || 'ทั้งหมด แยกตามสถานะ';
  var techLabel = printTechnicianFilter ? printTechnicianFilter : 'ทั้งหมด';
  var now = new Date();
  ...
  openPrintReportWindow({
    docTitle:    'รายงานแจ้งซ่อม',
    reportTitle: 'รายงานการแจ้งซ่อม',
    systemName:  'ระบบจัดการแจ้งซ่อม',
    metaRows: [
      { label: 'ช่วงเวลา',      value: periodLabel },
      { label: 'สถานะ',         value: statusLabel },
      { label: 'ผู้รับผิดชอบ',  value: techLabel },
